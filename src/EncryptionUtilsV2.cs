...
namespace Amazon.Extensions.S3.Encryption
{
    ...
    internal static partial class EncryptionUtils
    {
        ...
        /// </returns>
        internal static Stream EncryptRequestUsingAesGcm(Stream toBeEncrypted, EncryptionInstructions instructions, bool shouldUseCachingStream)
        {
            ...
            Stream gcmEncryptStream = (shouldUseCachingStream ? 
                                        new AesGcmEncryptCachingStream(toBeEncrypted, instructions.EnvelopeKey, instructions.InitializationVector, GenericUtils.ConvertByteToBit(instructions.AlgorithmSuite.AuthenticationTagLengthInBytes.Value))
                                        : new AesGcmEncryptStream(toBeEncrypted, instructions.EnvelopeKey, instructions.InitializationVector, GenericUtils.ConvertByteToBit(instructions.AlgorithmSuite.AuthenticationTagLengthInBytes.Value))
                                       );
            ...
            IAmazonKeyManagementService kmsClient, string kmsKeyId, KmsType kmsType, Dictionary<string,string> encryptionContextToKMS, AlgorithmSuite algorithmSuite)
        {
            ...
            switch (kmsType)
            {
                case KmsType.KmsContext:
                {
                    ...
                    var result = kmsClient.GenerateDataKey(kmsKeyId, encryptionContextToKMS, KMSKeySpec);

                    var instructions = new EncryptionInstructions(encryptionContextToKMS, result.KeyPlaintext, result.KeyCiphertext, iv,
            ...
            string kmsKeyId, KmsType kmsType, Dictionary<string,string> encryptionContextToKMS, AlgorithmSuite algorithmSuite)
        {
            ...
            switch (kmsType)
            {
                case KmsType.KmsContext:
                {
                    ...
                    // Generate iv, and get both the key and the encrypted key from KMS.
                    RandomNumberGenerator.Create().GetBytes(iv);
                    var result = await kmsClient.GenerateDataKeyAsync(kmsKeyId, encryptionContextToKMS, KMSKeySpec).ConfigureAwait(false);
        ...
        /// </returns>
        internal static EncryptionInstructions BuildInstructionsUsingInstructionFileV2(JsonData jsonData, EncryptionMaterialsBase materials)
        {
            if (jsonData[XAmzKeyV2] != null)
            {
                ...
                var algorithmSuit = GetAlgorithmSuitFromCekAlgValue(cekAlgorithm);
                var decryptedEnvelopeKey = DecryptNonKmsEnvelopeKeyV2V3(encryptedEnvelopeKey, materials, algorithmSuit);
                ...
                var instructions = new EncryptionInstructions(materialDescription, decryptedEnvelopeKey, null,
                    initializationVector, wrapAlgorithm, algorithmSuit);
