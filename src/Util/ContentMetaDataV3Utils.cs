...
namespace Amazon.Extensions.S3.Encryption.Util.ContentMetaDataUtils
{
    ...
    internal static class ContentMetaDataV3Utils
    {
        ...
        private const string XAmzWrapAlgAesGcmV3 = "02";
        private const string WrapAlgKmsContextV3 = "12";
        private const string XAmzWrapAlgRsaOaepSha1V3 = "22";
        ...
        /// <returns>True if V3 format is detected</returns>
        internal static bool IsV3ObjectInInstructionFileMode(JsonData instructionFileContent)
        {
            var hasEncryptedKey = instructionFileContent[EncryptedDataKeyV3] != null;
            var hasAlgorithm = instructionFileContent[EncryptedDataKeyAlgorithmV3] != null;
        ...
        internal static void ValidateV3InstructionFile(MetadataCollection objectMetadata,
            JsonData instructionFileContent)
        {
            //= ../specification/s3-encryption/data-format/content-metadata.md#content-metadata-mapkeys
            //# In the V3 format, the mapkeys "x-amz-c", "x-amz-d", and "x-amz-i" MUST be stored exclusively in the Object Metadata.
            ...
            if (instructionFileContent[ContentCipherV3] != null)
                throw new InvalidDataException(
            ...
            if (instructionFileContent[KeyCommitmentV3] != null)
                throw new InvalidDataException(
            ...
            if (instructionFileContent[MessageIdV3] != null)
            ...
            if (instructionFileContent[EncryptedDataKeyV3] == null)
            ...
            if (instructionFileContent[EncryptedDataKeyAlgorithmV3] == null)
                throw new InvalidDataException($"V3 instruction file requires {EncryptedDataKeyAlgorithmV3}");
            
            //= ../specification/s3-encryption/data-format/metadata-strategy.md#v3-instruction-files
            //= type=exception
            //= reason= x-amz-t is for kms wrapping key and S3EC .NET does not support it when on instruction file mode.
            ...
            if (instructionFileContent[EncryptionContextV3] != null)
        ...
        internal static void EnsureUniqueMetaDataForV3InInstructionFile(MetadataCollection objectMetadata, JsonData instructionFileContent)
        {
            //= ../specification/s3-encryption/data-format/content-metadata.md#determining-s3ec-object-status
            //= type=citation
            //# If there are multiple mapkeys which are meant to be exclusive, such as "x-amz-key", "x-amz-key-v2", and "x-amz-3" then the S3EC SHOULD throw an exception.                                                                                                                   
            ...
            if (EncryptionUtils.V1V2Keys.Any(key => instructionFileContent[key] != null))
